  ...
  return (
    <div className="page" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, background: "linear-gradient(135deg,#e8f0fe 0%,#d4e3fc 40%,#e8f5e9 100%)", position: "relative" }}>
      ...
      <div style={{ background: "#fff", borderRadius: "var(--radius)", overflow: "hidden", width: "100%", maxWidth: 420, boxShadow: "var(--shadow-lg)", animation: "fadeUp .5s .1s ease both" }}>
        ...
        { }
        <form autoComplete="off" onSubmit={e => { e.preventDefault(); handleLogin(); }} style={{ padding: 32, display: "flex", flexDirection: "column", gap: 20 }}>
          {error && (
          ...
          <div>
            ...
            <Input icon={<IcUser />} placeholder="correo@hospital.com" type="email" name="st_email_field" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleLogin()} autoComplete="username" />
          </div>
          <div>
            ...
            <Input icon={<IcLock s={16} c="var(--text-soft)" />} placeholder="••••••••" type="password" name="st_password_field" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleLogin()} autoComplete="new-password" />
          </div>
