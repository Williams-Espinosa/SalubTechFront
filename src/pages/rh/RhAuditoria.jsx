      ...
      {tab === "incidentes" && (
        <>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:20 }}>
            ...
            <button style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 18px", borderRadius:"var(--radius-sm)", background:"#fff", border:"1.5px solid var(--border)", fontSize:13, fontWeight:600, cursor:"pointer", marginLeft:"auto" }}>
              <IcDoc c="var(--text-mid)" s={14}/> Exportar Reporte
            </button>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
            {filtered.map((inc,i) => (
              <div key={i} style={{ background:"#fff", borderRadius:"var(--radius-sm)", padding:22, boxShadow:"var(--shadow-sm)", border:"1px solid var(--border)" }}>
        ...
        <div style={{ background:"#fff", borderRadius:"var(--radius-sm)", boxShadow:"var(--shadow-sm)", overflow:"hidden" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            <thead><tr style={{ background:"#FAFBFC" }}>
              {["Paciente","Cama","Eventos Reportados","Ficha"].map(h => <th key={h} style={{ padding:"12px 20px", textAlign:"left", fontSize:10, fontWeight:700, letterSpacing:1, textTransform:"uppercase", color:"var(--text-soft)", borderBottom:"1px solid var(--border)" }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {AUD_PATIENTS.map((p,i) => (
                <tr key={i}>
